import CodeMirror, {BasicSetupOptions, EditorView, Extension, ViewUpdate} from "@uiw/react-codemirror";
import {langs} from '@uiw/codemirror-extensions-langs'
import {useMemo, useState} from "react";
import useTheme from "../modules/useTheme.tsx";
import {LanguageKey, languages, useSortedLanguages} from "../modules/languageKey.ts";
...
function CodeEditor() {
  ...
  const [currentLanguage, setCurrentLanguage] = useState<LanguageKey>(settings.preferred_language)

  const onUpdateHandler = useMemo(() => ((update: ViewUpdate) => {
      ...
      <div className="flex justify-between" style={{
        ...
        <div className="m-2" >
          <select
              ...
              onChange={(e) => setCurrentLanguage(e.target.value as unknown as LanguageKey)}
          >
            { sortedLanguages.map(([key, lang]) =>  (
                <option key={key} value={key}>{lang.title}</option>
            ))}
          </select>
        </div>
