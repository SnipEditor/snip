...
function convertRgbToHsl(r: number, g: number, b: number) {
    ...
    if (cMax === cMin) {
        ...
        return { hue: 0, saturation: 0, luminosity: Math.round(luminosityPrime * 100) }
    }

    const saturationPrime = luminosityPrime > 0.5 ? delta / (2 - cMax - cMin) : delta / (cMax + cMin)
    let huePrime = 0
    switch (cMax) {
        case rPrime:
...
export default function getShades(baseColorHex: string, darkMode: boolean = false) {
    const baseHSL = convertHexToHsl(baseColorHex)
    console.log(baseHSL)
    const { hue: baseHue, saturation: baseSaturation, luminosity: baseLuminosity } = baseHSL
    ...
    return {
        '50': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (lowHalfMultiplier * 4.5), 0, 100)),
        '100': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (lowHalfMultiplier * 4), 0, 100)),
        '200': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (lowHalfMultiplier * 3), 0, 100)),
        '300': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (lowHalfMultiplier * 2), 0, 100)),
        '400': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + lowHalfMultiplier, 0, 100)),
        '500': baseColorHex,
        '600': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + highHalfMultiplier, 0, 100)),
        '700': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (highHalfMultiplier * 2), 0, 100)),
        '800': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (highHalfMultiplier * 3), 0, 100)),
        '900': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (highHalfMultiplier * 4), 0, 100)),
        '950': convertHslToHex(baseHue, baseSaturation, clamp(baseLuminosity + (highHalfMultiplier * 4.5), 0, 100)),
    }
}